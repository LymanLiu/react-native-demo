...
var $this = {
	...
	register: function(){
		...
		$('c!DM').register({
			user_name: this.refs.username.props.value,
			password: $.md5(this.refs.password.props.value),
			verified_phone: this.refs.mobilePhone.props.value,
			sub_PIN: this.refs.PIN.props.value,
	...
	sendPIN: function () {
		...
		.then(function(res){
			...
			if(res.data.status=='0'){
				$('#!tool').alert($.LN.REGISTER_RESPONDE,$.LN.SEND_MESSAGE_SUCCESS);
			}else{
				$('#!tool').alert($.LN.REGISTER_RESPONDE,res.data.msg);
			}
		});
	},
			...
			<ScrollView ref='scrollView' contentContainerStyle={styles.main}  >
			  	<View style={styles.uWrap}>
			  		...
			  		<TextInput
						...
						onFocus={this.inputFocused.bind(this, 'username',80) }
			  	...
			  	<View style={styles.bWrap}>
			        <View style={styles.bDivide}>
			  	  		<TouchableOpacity onPress={() => this.toLogin() } style={styles.bTauch}>
			            	<Text style={styles.bText}>{$.LN.LOGIN}</Text>
			      		</TouchableOpacity>
			        </View>
			        <View style={styles.bDivide}>
			      		<TouchableOpacity onPress={() => this.register() } style={[styles.bTauch,{backgroundColor:'#00B2EB',}]} >
			          		<Text style={[styles.bText,{color:'#FFFFFF',}]}>{$.LN.SUBMIT}</Text>
			      		</TouchableOpacity>
			        </View>
			    </View>
...
var Register = React.createClass($this);
